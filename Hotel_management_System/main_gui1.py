...
class Ui_MainWindow(object):
	...
	def insertdata(self):
		...
		try:
			...
			str1="insert into hotel values({},'{}',{},{},'{}','{}','{}')".format(self.room_no,name,member,mobileno,fromdate,todate,room_type)
			...
			for i in range(len1):
				...
				if int(s1[0])==self.room_no  and self.date_in_between(s1[4],s1[5],self.selected_date):
					...
					if(self.checkinputs(name,member,mobileno,fromdate,todate)):
						...
						#UPDATE `hotel` SET `room_no`=[value-1],`name`=[value-2],`total_member`=[value-3],`mobileno`=[value-4],`fromdate`=[value-5],`todate`=[value-6],`type`=[value-7] WHERE 1
						####################
						cn= mysql.connector.connect(
							user='root',
							password='',	
						...
						command = cn.cursor()
						str1="update hotel set name='{}',total_member={},mobileno={},fromdate='{}',todate='{}',type='{}' where room_no={} and fromdate='{}'".format(name,member,mobileno,fromdate,todate,room_type,self.room_no,s1[4])
	...
	def deletedata(self):
		...
		for i in range(len1):
			...
			if s1[0]!=str(self.room_no) or not(self.date_in_between(s1[4],s1[5],self.selected_date)):
				...
				command = cn.cursor()
				str1="delete from hotel where room_no={} and fromdate='{}'".format(self.room_no,s1[4])
